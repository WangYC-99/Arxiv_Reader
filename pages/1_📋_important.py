...
important_pubs = co_important.find()
...
def main():
    ...
    for each in important_pubs:
        show_st(each)
        st.button('move to interesting', key = each['_id'] + '.1', on_click = move_to_interesting, kwargs = dict(current_pub = each))
